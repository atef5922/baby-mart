  ...
  return (
    <section className="w-full">
      <div className="mx-auto flex w-full max-w-[1200px] flex-col items-center px-4 sm:px-6">
        <div className="relative mb-8 flex w-full flex-col items-center gap-3 text-center sm:mb-10">
          <Image
            src={newArrivalsLogo}
            alt="Baby Mart logo"
            width={186}
          ...
          <h2 className="mt-1 text-[2rem] font-black leading-[1.02] tracking-[-0.05em] text-transparent bg-[linear-gradient(90deg,#ff5ca8,#ff8b2c,#ffd54a,#67d8b4,#5bb5ff,#a27dff)] bg-clip-text sm:text-[2.7rem]">
            Crazy collections
          </h2>
        </div>
                ...
                <Link href={`/product/${product.slug}`} className="block" aria-label={`Open ${product.name}`}>
                  <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[12px] shadow-[0_4px_8px_rgba(0,0,0,0.06)]">
                    <Image
                      ...
                      sizes="(min-width: 1536px) 24vw, (min-width: 1024px) 30vw, (min-width: 768px) 48vw, 92vw"
                    />
                    {product.discount ? (
              ...
              <div className="p-6">
                ...
                <div className="mt-6 grid gap-3">
                  <button
                    ...
                    className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-[#7c3aed] px-5 text-sm font-semibold text-white transition duration-200 hover:bg-[#6d28d9]"
                  >
                    <ShoppingCart size={16} />
                    Add to Cart
                  </button>
                  <Link
                    href={`/product/${activeQuickProduct.slug}`}
                    className="inline-flex h-11 items-center justify-center rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
                    onClick={() => setActiveQuickProduct(null)}
                  >
                    View full details
                  </Link>
